...
// the child attr can access the parent function. higher order function. 
...
function animalNames(animals) {
  return animals.map(currentMapValue => `name: ${currentMapValue.animal_name}, scientific: ${currentMapValue.scientific_name}`);
}
...
function USApop(animals) {
  return animals.reduce((acc, cur) => acc + cur.population, 0)
}
